...
public class FastRK2 implements NumSolver {
    ...
    public void step(Model3D model, double dt) {
        ...
        // loop thru all values and do a step of euler with dt*alpha from vel to pos
        ...
        for(int i=0; i< model.size(); i++){
            ...
            for (int j=0; j<3; j++){
                // update position of each of them
                setVel(i,   this.getVel(i)[0] + this.getAcc(i)[0] * dt*alpha,
                            this.getVel(i)[1] + this.getAcc(i)[1] * dt*alpha,
                            this.getVel(i)[2] + this.getAcc(i)[2] * dt*alpha);
            }
        }
